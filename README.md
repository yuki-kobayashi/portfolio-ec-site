...
| 商品画像の拡大   | 商品購入画面にて商品の画像をクリックした際、JavaScriptで商品画像を拡大表示 |
...
- **Bootstrapのモーダル機能と、jQueryによるAJAX処理を組み合わせたお気に入り機能**を実装し、ページ遷移のないスムーズな動きを実現。
```javascript
// お気に入り切り替え(非同期更新)
...
- **決済機能の本番実装**：Stripeを用いた決済処理については、カード情報の取り扱いのリスクを考慮し未実装としましたが、Stripeには開発者向けのテスト環境での実装ができるという情報を得たため、将来決済機能を付けたサイトの開発を行う機会があれば、テスト環境を活用して実装に挑戦したいと考えています。
- **ユニットテストの自動化**：本サイトの開発においては、手動での動作確認を行いました。しかし折角Pythonでの開発でもあるため、pytestによる自動テスト導入を行ってみたいです。
- **UI/UXの改善**：Bootstrapを活用したスタイル設計は実装できましたが、あまり一貫性を考えずに実装を行っていました。今後はFigmaなどでワイヤーフレームを作成し、より一貫性のある設計を目指したいと考えています。
