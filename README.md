...
- **決済機能の本番実装**：Stripeを用いた決済処理については、カード情報の取り扱いのリスクを考慮し未実装としましたが、Stripeには開発者向けのテスト環境での実装ができるという情報を得たため、将来決済機能を付けたサイトの開発を行う機会があれば、テスト環境を活用して実装にチャレンジしたいと考えています。
- **ユニットテストの自動化**：本サイトの開発においては、手動での動作確認を行いました。しかし折角Python + Djangoでの開発でもあるため、`pytest`や`Django TestCase`による自動テスト導入を進めたいです。
